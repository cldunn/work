...
import { AfwForm, AfwFormInput, required } from "afw-components";
...
const LoginView = (): JSX.Element => {
    ...
    const onSubmit = async (values: any) => {
        ...
        const _password = pKey.encrypt(password, 'RSA-OAEP', {
        ...
        sessionStorage.setItem("Authorization", encodeB64(values.name + ':' + values.password));
        
        try {
            await dispatch(login({
                url: '/login',
                headers: {
                    Authorization: 'Basic ' +  encodeB64(values.name + ':' + values.password)
                },
                params: {
                    name: values.name,
