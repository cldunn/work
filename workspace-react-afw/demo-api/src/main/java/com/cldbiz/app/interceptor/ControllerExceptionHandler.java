package com.cldbiz.app.interceptor;
...
import com.cldbiz.app.common.JsonResponse;
import com.cldbiz.app.exception.ApplicationException;
...
public class ControllerExceptionHandler extends ResponseEntityExceptionHandler {
	...
	public ResponseEntity<JsonResponse> handleApplicationExcpetion(ApplicationException appEx, WebRequest request) {
		...
		if (appEx.getValidationMsgs().size() > 0) {
			jsonResp = JsonResponse.getBuilder(appEx.getValidationMsgs())
			...
			jsonResp = JsonResponse.getBuilder(contentMsg, appEx.getDetails())
					.addData(appEx.getData())
					.built();
		}
