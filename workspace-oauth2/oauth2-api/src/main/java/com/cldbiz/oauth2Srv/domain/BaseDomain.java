...
import com.cldbiz.oauth2Srv.common.AppConstants;
import com.cldbiz.oauth2Srv.config.ExecutionContext;
...
public abstract class BaseDomain implements Serializable {
	...
	public void insertCreatedByAndCreatedDate() {
		UserDetail userDetail = ExecutionContext.getUserDetail();
		
		if (userDetail != null) {
			this.setCreatedBy(userDetail.getUsername());
		}
		else {
	...
	public void updateMaintainedByAndModifiedDate() {
		...
		if (userDetail != null) {
			this.setMaintainedBy(userDetail.getUsername());
		}
		else {
			this.setCreatedBy(AppConstants.SYSTEM_USER);
		}
